...
const perms={full:['stats','bookings','offers','ads','gallery'],pricing_ads:['offers','ads','gallery']};
let current=null;
...
async function loadBookings(){try{const b=await rpc('yz_admin_bookings',authArgs());const arr=Array.isArray(b)?b:[];$('bookingsList').innerHTML=arr.map(x=>`<div class="booking"><div class="booking-top"><div><b>${esc(x.customer_name||'بدون اسم')}</b><small>${esc(x.booking_code||'')}</small></div><div class="booking-actions"><select onchange="setStatus(${x.id},this.value)"><option value="new" ${x.status==='new'?'selected':''}>جديد</option><option value="confirmed" ${x.status==='confirmed'?'selected':''}>مؤكد</option></select><button class="cancel-booking" onclick="cancelBooking(${x.id})" ${x.status==='cancelled'?'disabled':''}>إلغاء الحجز وحذفه من الموقع</button></div></div><div class="booking-grid"><span>الهاتف<br><b>${esc(x.customer_phone||'-')}</b></span><span>التاريخ<br><b>${esc(x.booking_date||'-')}</b></span><span>العدد<br><b>${x.guests||0}</b></span><span>الخدمة<br><b>${esc(x.service_type||'-')}</b></span></div>${x.edit_count>0?`<div class="edited-badge">تم تعديل الحجز ${x.edit_count} ${x.edit_count===1?'مرة':'مرات'} · تاريخ إنشاء الطلب محفوظ</div>`:''}${x.notes?`<div class="notes">${esc(x.notes)}</div>`:''}</div>`).join('')||'<div class="empty">لا توجد حجوزات</div>'}catch(e){$('bookingsList').innerHTML='<div class="error">تعذر تحميل الحجوزات</div>'}}
async function setStatus(id,status){try{await rpc('yz_admin_booking_status',{...authArgs(),p_id:id,p_status:status});await loadBookings();await loadDashboard()}catch(e){alert('تعذر تحديث حالة الحجز')}}
async function cancelBooking(id){if(!confirm('هل أنت متأكد من إلغاء هذا الحجز؟'))return;try{await rpc('yz_admin_booking_status',{...authArgs(),p_id:id,p_status:'cancelled'});await loadBookings();await loadDashboard()}catch(e){alert('تعذر إلغاء الحجز وحذفه من الموقع')}}
...
async function editOffer(id){
 try{
  ...
  $('offerId').value=x.id;$('offerNameAr').value=x.name_ar||'';$('offerNameEn').value=x.name_en||'';$('offerDescAr').value=x.description_ar||'';$('offerDescEn').value=x.description_en||'';$('offerCategory').value=x.category||'general';$('offerSort').value=x.sort_order??0;$('offerPrice').value=x.price??0;$('offerActive').checked=!!x.active;$('offerBookingEnabled').checked=x.booking_enabled!==false;$('offerEditor').classList.remove('hidden');$('offerMsg').textContent=''
 }catch(e){alert('تعذر فتح الباقة')}
}
async function deleteOffer(id){
 if(!confirm('حذف هذه الباقة نهائيًا؟'))return;
 try{await rpc('yz_admin_offer_delete',{...authArgs(),p_id:Number(id)});await loadOffers()}
 catch(e){alert('تعذر حذف الباقة: '+e.message)}
}
async function saveOffer(){
 const id=$('offerId').value;
 const body={...authArgs(),p_id:Number(id),p_name_ar:$('offerNameAr').value.trim(),p_name_en:$('offerNameEn').value.trim(),p_description_ar:$('offerDescAr').value.trim(),p_description_en:$('offerDescEn').value.trim(),p_price:Number($('offerPrice').value)||0,p_active:$('offerActive').checked,p_category:$('offerCategory').value,p_sort_order:Number($('offerSort').value)||0,p_booking_enabled:$('offerBookingEnabled').checked};
 if(!body.p_name_ar&&!body.p_name_en){$('offerMsg').textContent='اكتب اسم الباقة';return}
 try{
  if(id){await rpc('yz_admin_offer_update',body)}
  else{await rpc('yz_admin_offer_create',{...authArgs(),p_name_ar:body.p_name_ar,p_name_en:body.p_name_en,p_description_ar:body.p_description_ar,p_description_en:body.p_description_en,p_price:body.p_price,p_active:body.p_active,p_category:body.p_category,p_sort_order:body.p_sort_order,p_booking_enabled:body.p_booking_enabled})}
...
async function deleteAd(id){if(!confirm('حذف الإعلان أو الفعالية نهائيًا؟'))return;try{await rpc('yz_admin_ad_delete',{...authArgs(),p_id:id});await loadAds()}catch(e){alert('تعذر حذف الإعلان: '+e.message)}}
...
async function saveAd(){
 ...
 const body={...authArgs(),p_id:$('adId').value||null,p_title:$('adTitle').value.trim(),p_text:$('adBody').value.trim(),p_image_url:$('adImage').value.trim(),p_active:$('adActive').checked,p_bookable:$('adBookable').checked,p_price:Number($('adPrice').value)||0};
