...
function resizeImage(file){return new Promise((resolve,reject)=>{const r=new FileReader();r.onload=()=>{const img=new Image();img.onload=()=>{const max=1400,scale=Math.min(1,max/Math.max(img.width,img.height));const c=document.createElement('canvas');c.width=Math.max(1,Math.round(img.width*scale));c.height=Math.max(1,Math.round(img.height*scale));const ctx=c.getContext('2d');ctx.drawImage(img,0,0,c.width,c.height);resolve(c.toDataURL('image/jpeg',.68))};img.onerror=()=>reject(new Error('تعذر قراءة الصورة'));img.src=r.result};r.onerror=()=>reject(new Error('تعذر قراءة ملف الصورة'));r.readAsDataURL(file)})}
async function saveAd(){
 const file=$('adFile').files[0];
 if(file&&file.size>8*1024*1024){$('adMsg').textContent='الصورة كبيرة. اختر صورة أقل من 8MB.';return}
 ...
 $('adMsg').textContent='جارٍ حفظ الإعلان...';
 try{
   if(file) body.p_image_url=await resizeImage(file);
   const result=await rpc('yz_admin_ad_upsert',body);
   if(!result) throw new Error('لم يُرجع الخادم نتيجة للحفظ');
   $('adEditor').classList.add('hidden');$('adMsg').textContent='';await loadAds();
 }catch(e){
   console.error('saveAd error',e);
   let msg=e?.message||e?.error_description||e?.details||e?.hint||'';
   if(typeof msg==='object') msg=JSON.stringify(msg);
   if(!msg||msg==='undefined') msg='حدث خطأ غير معروف أثناء الحفظ. تحقق من الاتصال بقاعدة البيانات ثم حاول مرة أخرى.';
   try{const parsed=JSON.parse(msg);msg=parsed.message||parsed.error||parsed.hint||parsed.details||msg}catch(_){}
   $('adMsg').textContent='تعذر الحفظ: '+msg;
 }
}
